...
{
    public class CsvToParquetConverterService : ICsvToParquetConverterService
    {

        private readonly ILogger<CsvToParquetConverterService> _logger;
        ...
        public CsvToParquetConverterService(
           ILogger<CsvToParquetConverterService> logger, ISMBLibraryServices ismbLibraryServices, IProcessConfigurationRepository iIProcessConfigurationRepository, ICache cache)
        {
            _logger = logger;
        ...
        public async Task<ParquetFileResponseDto> ConvertDataToParquet(ConfigurationTableMappingDto configurationTableMappingDto, FlpConfigurationResponseDto parameterConfig, FlpProcessTempFile fileProcessingConfig)
        {
            ...
            try
            {
                ...
                // string blobConnectionString = fileProcessingConfig.BlobConnectionString;
                BlobServiceClient blobServiceClient = new BlobServiceClient(fileProcessingConfig.ParquetBlobConnectionString);

                // Get the container client for the CSV source
                ...
                {
                    // Get the container client for the Parquet file destination
                    // BlobContainerClient parquetContainerClient = blobServiceClient.GetBlobContainerClient(fileProcessingConfig.ParquetBlobConnectionString);
                    ...
                    using (var parquetStream = await parquetBlobClient.OpenWriteAsync(overwrite: true))
                    {
                        ...
                        resultResponse = await ConvertCsvToParquetAsync(csvStream, parquetStream, configurationTableMappingDto);
        ...
        private async Task<ParquetFileResponseDto> ConvertCsvToParquetAsync(Stream csvStream, Stream parquetStream, ConfigurationTableMappingDto parameterConfig)
        {
            ...
            var isValidFile = IsValidDelimiter(csvStream, parameterConfig.Delimiter);
            if (!isValidFile)
            {
                        ...
                        for (int i = 0; i < columns.Count; i++)
                        {
                            ...
                            else if (columnDataTypeMappings != null)
                            {
                                ...
                                var dataColumn = dataType switch
                                {
                                    ...
                                    "double" => new DataColumn(dataField, columns[i].Select(x =>
                                    {
                                        double doubleValue;
                                    ...
                                    _ => new DataColumn(dataField, columns[i].Select(x => x?.ToString() ?? string.Empty).ToArray())
                                };
